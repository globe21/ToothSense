...
public class EPContactsPicker: UITableViewController, UISearchResultsUpdating, UISearchBarDelegate, NavgationTransitionable {
    
    
    public var tr_pushTransition: TRNavgationTransitionDelegate?
    ...
    var multiSelectEnabled: Bool = true //Default is single selection contact
    
    // MARK: - Lifecycle Methods
    
    public override func viewDidAppear(animated: Bool) {
        if !orderedContacts.isEmpty {
            let counter = (orderedContacts.count - 1)
            for i in 0..<counter {
    ...
    func initializeSearchBar() {
        self.resultSearchController = ( {
            ...
            controller.searchBar.frame = CGRect(x: 0, y: 0, width: self.view.frame.width, height: 45)
            controller.searchBar.barTintColor = AppConfiguration.navColor
            controller.searchBar.tintColor = AppConfiguration.navText
    ...
    func inititlizeBarButtons() {
        //self.navigationController?.navigationBar.titleTextAttributes = [NSForegroundColorAttributeName : UIColor.whiteColor(), NSFontAttributeName : UIFont(name: "AmericanTypewriter-Bold", size: 20)!]
        let cancelButton = UIBarButtonItem(title: "Done", style: UIBarButtonItemStyle.Plain, target: self, action: #selector(onTouchCancelButton))
        cancelButton.setTitleTextAttributes([NSForegroundColorAttributeName : UIColor.whiteColor(), NSFontAttributeName : UIFont(name: "AmericanTypewriter-Bold", size: 20)!], forState: .Normal)
        self.navigationItem.leftBarButtonItem = cancelButton
        if multiSelectEnabled {
            let doneButton = UIBarButtonItem(title: "Send", style: UIBarButtonItemStyle.Plain, target: self, action: #selector(onTouchDoneButton))
            doneButton.setTitleTextAttributes([NSForegroundColorAttributeName : UIColor.whiteColor(), NSFontAttributeName : UIFont(name: "AmericanTypewriter-Bold", size: 20)!], forState: .Normal)
    ...
    public func updateSearchResultsForSearchController(searchController: UISearchController) {
        if let searchText = resultSearchController.searchBar.text  where searchController.active {
            ...
            do {
                filteredContacts = try store.unifiedContactsMatchingPredicate(predicate, keysToFetch: allowedContactKeys())
                self.tableView.reloadData()
            }
            catch {
                print("Error!")
