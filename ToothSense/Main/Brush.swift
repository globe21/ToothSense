...
class Brush: UIViewController, UINavigationControllerDelegate, NavgationTransitionable {
    ...
    func createVideo(playerController: AVPlayerViewController, url: NSURL) {
        ...
        if playerController == self.playerController {
            playerController.view.frame = CGRect(x: 5, y: 20, width: self.view.frame.width - 10, height: self.view.frame.width * (9/16))
        } else {
            playerController.view.frame = CGRect(x: 5, y: self.playerController.view.frame.maxY + 15, width: self.view.frame.width - 10, height: self.view.frame.width * (9/16))
        }
